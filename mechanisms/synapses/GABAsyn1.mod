...
- Set Tmax so that the peak Tmax*(B_GABAB-A_GABAB) = 1.0
    ...
    + this can be tested by copying time constants from GABA-B to GABA-A,
      setting gmax_GABAA=1, and observing the peak g_GABAA value (at max
      saturation, when facilitation is complete)
...
NEURON {
    ...
    RANGE R : state variables for GABAB-B dynamics
    RANGE K1, K2, Tmax : parameters for GABA-B dynamics
...
PARAMETER {
    ...
    : Good settings seem (tau_r, tau_f) = (5, 100) and (200, 200)
...
: Parameters for kinetic model GABA-B signaling cascade
    K1  = 5.2                 : forward binding rate to receptor
    K2  = 0.013               : backward (unbinding) rate of receptor
    Tmax = 1.0                : peak neurotransmitter concentration
}
...
DERIVATIVE odes {
    ...
    : We just low-pass filter the transmitter concentration, which should be set
    : with facilitating parameters
    R' = K1 * Tmax*(B_GABAB-A_GABAB) * Rfactor - K2 * R
