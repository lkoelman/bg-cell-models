...
- [Bevan (2017)](Review chapter 'The Subthalamic Nucleus' in Handbook of Basal Ganglia Structure & Function)
	...
	+ Unitary GABAAR-mediated `GPe-STN conductances` are large (`~5-15 nS`) because each axon forms multiple synaptic inputs with an individual postsynap-tic neuron
		* (Atherton et al., 2013; Baufreton et al., 2009; Smith et al., 1990). 

	+ However, sustained presynaptic GPe-STN activity is associated with _profound short-term synaptic depression_ due to depletion of release-ready vesicles, which limits the reliability and ampli-tude of unitary synaptic transmission (Atherton et al.,2013)
	...
	+ GPe-STN inputs also act through postsynaptic GABA-B receptors, which underlie a slow inhibitory postsynaptic potential (IPSP) that reverses at the equilibrium potential for K+.
...
- [Baufreton et al. 2009](Sparse but Selective and Potent Synaptic Transmission From the Globus Pallidus to the Subthalamic Nucleus)
	...
	- [Fig. 3]() : Three-dimensional distribution of GP-STN terminals
		- TODO

	- [Fig. 5](panel A, top traces, black) GABA-A synapse IPSP characteristics
		- Imax = ~ 375-680 pA
		- tau = ~ 6-9 ms (time from peak to 1/e = 36.8% of peak)

	- [Fig. 7](panel C)  population data for the peak hyperpolarization generated by each IPSP

		- responses to IPSPs with conductances of `5-25 nS` were recorded:

			| gbar (nS)      | 0    | 5    | 10    | 15    | 20    | 25  |
			|----------------|------|------|-------|-------|-------|-----|
			| IPSP_peak (mV) | -71  | -76  | -78.5 | -79   | -81   | -82 |
			| IPSP_ampl (mV) | -1.5 | -6.5 | -8.5  | -10.5 | -11.5 | -12 |
			

- [Atherton et al (2013)](Short-term Depression of External Globus Pallidus-Subthalamic Nucleus Synaptic Transmission and Implications for Patterning Subthalamic Activity)

	+ Fig. 1: Unitary GPe–STN synaptic transmission is subject to profound short-term depression at in vivo rates of activity

	+ Fig. 2: Unitary GPe–STN synaptic transmission recovers slowly from short-term synaptic depression

	+ Fig. 3: Activity dependence of unitary GPe–STN synaptic transmission.
...
- [Fan (2012)](Proliferation of External Globus Pallidus-Subthalamic Nucleus Synapses following Degeneration of Midbrain Dopamine Neurons)
	...
	+ [Fig. 1](A-D) : Chronic dopamine depletion increased the frequency and prolonged the decay
kinetics of mIPSCs 
	
		* *NOTE* (definition of mIPSC vs sIPSC) "Spontaneous postsynaptic currents are currents generated by action-potential-dependent and –independent release of neurotransmitter in the absence of experimental stimulation. Miniature postsynaptic currents are currents observed in the _absence of presynaptic action potentials_; they are thought to correspond to the response that is elicited by a _single vesicle_ of transmitter."  

		* A, Representative recordings of mIPSCs in STN neurons derived from control and 6-OHDA-treated animals. The frequency but not the amplitude of mIPSCs was greater following dopamine depletion. 

		* B, Cumulative probability (cum. prob.) plots of the sample populations confirmed that dopamine depletion significantly reduced the intervals between mIPSCs but had no effect on the conductance underlying mIPSCs. 

		* C, The mean frequency of mIPSCs was significantly enhanced by dopamine depletion, whereas the mean conductance underlying mIPSCs was unaltered. 
			- `max frequency` increased from `~1.6 Hz` to `~3.4 Hz`
			- `mean conductance (mIPSC)` increased from `~0.6125 nS` to `~0.75 nS`

		* D, The decay of mIPSCs was prolonged by chronic dopamine depletion, as evidenced by representative mean peak-scaled mIPSCs and box plots."
			- `tau_decay` increased from `~6 ms` to `~8 ms`

	+ [Fig. 2]() : Chronic dopamine depletion increased the magnitude of evoked GABAA receptor-mediated synaptic transmission but had no effect on short-term synaptic plasticity
		* (10 Hz input spike train) `conductance of first sIPSC` is `~7 nS` in control, increaseds to `~11.5 nS` in 6-OHDA rat
		* (50 Hz input spike train) `conductance of first sIPSC` is `~7.5 nS` in control, increaseds to `~12 nS` in 6-OHDA rat

	+ [Table 2]() :  Chronic dopamine depletion increases the maximum evoked GABAA receptor-mediated synaptic conductance but has no effect on short-term synaptic plasticity
		* control rat `IPSC1` is `7.03 +/- 3.10 nS`
		* 6-OHDA rat `IPSC1` is `11.17 +/- 5.41 nS`

	+ [Fig. 3](A,B) : Chronic dopamine depletion increased the conductance evoked by the GABA-A receptor agonist isoguvacine
		+ conductance increased: `mean conductance` increases from `~4 nS` to `~8 nS`
...
- `Galvan (2015)`
	...
	- After DA depletion, the mean firing rate of STN neurons is increased (primates/rodents)
...
- `Wilson, Bevan (2011)`
	...
	- The STN has three main firing patterns (in vivo):
		...
		- 1) _Regular single spiking_ pattern (12% cells of humans w/ essential tremor, 22% of anaesthesized rodents). 
		
		- 2) _Irregular pattern_ w/ long ISI and sporadic _doublets and triplets_ (52% cells of humans w/ essential tremor, 40-70% anaesthesized rodents). 
		
		- 3) _Bursting_ (irregular or rhytmic) (36% cells humans w/ tremor 3-30% anaestesized rodents. In PD the % bursting cells increases and bursts become stronger and more rhytmic.
...
- `Gillies, Willshaw (2006)`

	- The generation of long bursts under reduced sKCa conductance conditions is comparable with the observation that apamin can extend burst duration in neurons that exhibit short bursts (Hallworth et al. 2003)
...
- comparison of Otsuka - Rubin & Terman - Gillies & Willshaw model:
