"""
Interaction with and visualization of morphologies.

This module was copied from PyNeuron-Toolbox: Alex H Williams. (2014). PyNeuron-Toolbox. GitHub Repository. https://github.com/ahwillia/PyNeuron-Toolbox DOI:10.5281/zenodo.12576
"""
...
# def get_segment_paths(sec):
#   """
#   Get 2D array per segment, containing the 3D points associated with that segment.
#   Returns:
#       interp_xyz : list(np.array)
#       One 2D array per segment, containing the 3D points in xyz associated
#       with each segment.
#   """
#   # TODO: just check that cartesian distance between 3D point and Section start lies between segment boundaries
#   pass # same as interpolate_jagged
...
def get_section_path(h,sec):
    """
    Get Section's 3D points as 2D NumPy array

    Returns:
        path : np.array(h.n3d(sec), 3)
        Two-dimensional array with XYZ points as rows     
    """
    n3d = int(h.n3d(sec=sec))
    if n3d == 0:
        raise ValueError("Section {} has no 3D shape information associated. "
                        "Assign 3D points first or call h.define_shape() to "
                        "assign based on L and diam information.".format(sec))
    xyz = []
