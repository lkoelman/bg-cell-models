...
`mpirun -n 6 python model_parameterized.py --scale 0.5 --dur 3000 --dnorm --nolfp --seed 888 --transientperiod 0.0 --writeinterval 3000 -id CALDNORM --outdir ~/storage/BBB_LuNetStnGpe --config configs/StnGpe_template_syn-V8.json 2>&1 | tee CALDNORM.log`

To run from an IPython shell, use the %run magic function like:

`%run model_parameterized.py --scale 0.5 --dd --dur 100 --seed 888 --transientperiod 0.0 --writeinterval 1000 --nogui -id test1 --outdir ~/storage --config configs/syn-V18_template.json`
...
def run_simple_net(
    ...
    stn_type = gillies.StnReducedType(
    ...
    proto_type = gunay.GpeReducedType(**gpe_common_params)
