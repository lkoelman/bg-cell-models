...
NEURON {
	...
	RANGE g, ikv				: Potassium current, specific conductance and equilibrium potential
}

PARAMETER {
	ek = -90 (mV)
	ikv = 0.0 (mA/cm2)				: Parameter to record this current separately to total sodium current
	g = 0.6e-3 (S/cm2)
...
BREAKPOINT {
	...
	ik = g*n*(v - ek)
	ikv = ik 						: Record ikv (just this potassium current) to check it is working
}
