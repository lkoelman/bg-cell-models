...
NEURON {
	SUFFIX Kd_Foust
	USEION k WRITE ik			: Using k ion, treat the reversal potential as a
								: parameter and write to ik so the total k 
								: current can be tracked
	RANGE g, ikd				: Potassium current, specific conductance and 
								: equilibrium potential
}

PARAMETER {
	ek = -90 (mV)
	ikd = 0.0 (mA/cm2)			: Parameter to record this current separately to
	...
	q_m = 8
	q_h = 7.3
	Q_s = 3.209					: Temperature rescaling - Q_10 = 2.3 
								: => Q_s = (Q_10)^((37-23)/10) = 3.209
}
...
BREAKPOINT {
	...
	ik = g*m*h*(v - ek)
	ikd = ik 					: Record ikd (just this potassium current) 
